...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>HTML詩語辞典 — タグ標本箱</title>
<meta name="description" content="HTML5 のタグを昆虫標本のように一匹ずつ並べた標本箱。眺め、観察し、二匹を交配させて詩の種を得る——素のHTMLで詩をつくるための辞書、その表紙。">
<meta property="og:type" content="website">
<meta property="og:site_name" content="HTML詩語辞典">
<meta property="og:title" content="HTML詩語辞典 — タグ標本箱">
<meta property="og:description" content="HTML5 のタグを標本のように並べ、眺め・観察し・交配させる。素のHTMLで詩をつくるための辞書。">
...
<style>
  * { box-sizing:border-box; }
  ...
  .head .kicker { margin:0 0 .2rem; font-size:.72rem; letter-spacing:.34em; color:#b6a589; }
  .head h1 { font-size:1.5rem; letter-spacing:.2em; margin:.1rem 0 .2rem; color:#f2ead9; }
  ...
  .lbl { position:relative; text-align:center; font-size:.56rem; color:#45301a;
    background:
  ...
  .card .det { display:block; margin-top:3px; padding-top:3px;
    ...
    color:#5a4124; font-style:italic; }
  ...
  .st-recens  .card .det { color:#3a6a93; }
  .st-relicta .card .det { color:#a8442a; }
  .st-nova    .card .det::before, .legend .k-nova    i { background:#5f8a5a; }
  .st-recens  .card .det::before, .legend .k-recens  i { background:#4a78a4; }
...
<body>
  <header class="head">
    <p class="kicker">ＨＴＭＬ　詩語辞典</p>
    <h1>タグ標本箱</h1>
    <p>HTML5 のタグを一匹ずつ採集し、目(Order)ごとに並べた標本ケース。<br>
    ただ眺めて、愛でるためのもの。これまでに ${specimens.length} 種を収めました。</p>
    <p class="head-hint" hidden>標本をクリック（または Enter）で<b>観察票</b>。<b>← →</b>・スワイプでめくり、<b>収蔵</b>で集め、二匹を<b>交配</b>させると詩の問いが立ちます。</p>
    <p class="legend">
...
${drawers}
  ...
  (function(){
    ...
    // 辞書モード(dictionary.html)と共有する保存領域。識別子は分割した正準タグ。
    var COLL_KEY='htmldict.collection.v1', CROSS_KEY='htmldict.crossing.v1';
    function _readArr(key){ try{ var v=JSON.parse(localStorage.getItem(key)||'null'); return Array.isArray(v)?v:null; }catch(e){ return null; } }
    function _migrate(nk, ok){ try{ if(localStorage.getItem(nk)===null){ var o=localStorage.getItem(ok); if(o!==null) localStorage.setItem(nk,o); } }catch(e){} }
    _migrate(CROSS_KEY,'htmldict.case.cross'); _migrate(COLL_KEY,'htmldict.case.kept');
    var cross=(_readArr(CROSS_KEY)||[]).filter(function(t){ return byTag[t]; });
    function saveCross(){ try{ localStorage.setItem(CROSS_KEY,JSON.stringify(cross)); }catch(e){} }
    function deco(x){ return String(x).replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&'); }

    // しおり（収蔵）
    var kept=new Set();
    (_readArr(COLL_KEY)||[]).forEach(function(t){ if(byTag[t]) kept.add(t); });
    function saveKept(){ try{ localStorage.setItem(COLL_KEY,JSON.stringify(Array.from(kept))); }catch(e){} }
    ...
    var _lastFocus=null, _hashOn=false;
    ...
    function openLoupe(tag, list, opts){
      opts=opts||{};
      ...
      if(opts.fromHistory){ _hashOn=true; } else { try{ history.pushState({loupe:tag}, '', '#'+encodeURIComponent(tag)); _hashOn=true; }catch(e){} }
      try{ if(multi && !localStorage.getItem('htmldict.tip.flip')){ localStorage.setItem('htmldict.tip.flip','1'); toast('‹ › ・ ← → ・ スワイプで、隣の標本へめくれます'); } }catch(e){}
    }
    function navTo(d){
      if(navList.length<2) return;
      navIndex=(navIndex+d+navList.length)%navList.length;
      fillLoupe(navList[navIndex]);
      try{ history.replaceState({loupe:navList[navIndex]}, '', '#'+encodeURIComponent(navList[navIndex])); }catch(e){}
    ...
    function closeLoupe(opts){ opts=opts||{}; loupe.hidden=true; if(document.getElementById('keptCase').hidden) document.body.classList.remove('loupe-open'); if(_lastFocus&&_lastFocus.focus){ try{ _lastFocus.focus(); }catch(e){} } if(_hashOn && !opts.fromHistory){ _hashOn=false; try{ history.back(); }catch(e){} } else { _hashOn=false; } }
    ...
    // ディープリンクと戻る: #タグ で観察票を開く／戻るで閉じる
    window.addEventListener('popstate', function(){
      if(!loupe.hidden){ closeLoupe({fromHistory:true}); return; }
      var t=decodeURIComponent((location.hash||'').replace(/^#/,''));
      if(t && byTag[t]) openLoupe(t, caseList(), {fromHistory:true});
    });
    ...
    // 初期ロード時に #タグ があれば、その観察票を開く（履歴は基底→#タグに整える）
    (function(){
      var t=decodeURIComponent((location.hash||'').replace(/^#/,''));
      if(t && byTag[t]){ try{ history.replaceState(null, '', location.pathname+location.search); }catch(e){} openLoupe(t, caseList()); }
    })();
  })();
  </script>
</body>
